...
while True:
	...
	if len(results_canny) > 0:
		...
		if (classID == 2) or (classID == 3) or (classID == 4) or (classID == 5) or (classID == 6) or (classID ==7):
			...
			if (score_canny > .85) and ((current_time - last_time) > 2.2):		     # Lets find successes	
				...
				print("RGB Image classification", "{:.6f}".format(duration_canny),classID_canny,"{:.3f}".format(score_canny))
				...
				insert_row_data = [dt_object,duration_canny,int(str(classID_canny)),score_canny]
